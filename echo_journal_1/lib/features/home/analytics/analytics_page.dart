...
class _AnalyticsPageState extends State<AnalyticsPage>
  ...
  List<Map<String, dynamic>> _getFilteredData(String timeFrame) {
    ...
    switch (timeFrame) {
      ...
      case 'Monthly':
        ...
        break;
      case 'Yearly':
        // All time data - will be filtered by year in the chart
    ...
    final filteredData = _sentimentData.where((item) {
      ...
      try {
        journalDate = DateTime.parse(item['date']);
      } catch (e) {
        print(
          'Error parsing date in _getFilteredData: ${item['date']} - $e',
        );
        return false;
      }
    ...
    print(
      '$timeFrame filter: Total entries: ${_sentimentData.length}, Filtered: ${filteredData.length}',
    );
  ...
  List<FlSpot> _generateLineChartData(
    ...
    // Group by day or week or month based on timeframe
    ...
    for (var item in data) {
      ...
      // Use appropriate grouping based on timeframe
      switch (timeFrame) {
        case 'Weekly':
          key = DateFormat(
            'yyyy-MM-dd',
          ).format(date); // Group by day for weekly
          break;
        case 'Monthly':
          key = DateFormat(
            'yyyy-MM',
          ).format(date); // Group by month for monthly
          break;
        case 'Yearly':
          key = DateFormat('yyyy').format(date); // Group by year for yearly
          break;
        default:
    ...
    // Create spots with normalized x values
  ...
  Widget _buildTimeFrameAnalytics(
    ...
    if (filteredData.isEmpty) {
      return Center(
        child: Text('No mood data available for the selected $timeFrame'),
      );
    }

    final distribution = _calculateSentimentDistribution(
      filteredData,
      timeFrame,
    );
  ...
  Widget _buildLineChart(
    ...
    Widget bottomTitleWidgets(double value, TitleMeta meta) {
      ...
      if (index >= spots.length || index < 0 || index >= dateLabels.length)
        return const SizedBox.shrink();
      ...
      try {
        ...
        if (timeFrame == 'Weekly') {
          final date = DateTime.parse(dateLabels[index]);
          title = DateFormat('EEE').format(date); // Day of week for weekly
        } else if (timeFrame == 'Monthly') {
          title = dateLabels[index].substring(5); // Show month only (MM)
        } else if (timeFrame == 'Yearly') {
          title = dateLabels[index]; // Show full year (YYYY)
        }
      } catch (e) {
